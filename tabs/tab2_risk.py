...
The "What is the worst-case scenario?" view.
"""
...
def render(data: pd.DataFrame, cfg: dict) -> None:
    ...
    with c2:
        metric_with_help("Max Drawdown", f"{dd['max_dd']*100:.1f}%",
                         "Largest peak-to-trough percentage decline in the period.")
    with c3:
    ...
    # ── EWMA Conditional Volatility (GARCH-lite) ────────────────────────────
    section_header("Conditional Volatility (EWMA / GARCH-lite)",
                   "RiskMetrics λ=0.94 EWMA. Captures volatility clustering like GARCH(1,1).")
    ...
    # ── VaR / CVaR table ────────────────────────────────────────────────────
    section_header("Tail Risk Metrics", "Negative return thresholds at various confidence levels.")
    ...
    st.dataframe(
        pd.DataFrame(rows).set_index("Confidence"),
        use_container_width=True,
    )
    callout(
        "VaR tells you the <b>minimum</b> loss in the worst scenarios. "
        "CVaR (Expected Shortfall) is the <b>average</b> loss beyond that threshold — "
        "a more conservative risk measure.",
        "info",
    )
    ...
    fig_dd.update_layout(**PLOTLY_LAYOUT, height=250,
    ...
    section_header("Return Distribution",
                   "Histogram of daily returns overlaid with a fitted normal curve.")
    ...
    y_norm = norm.pdf(x_range, mu_r, sigma_r)
    ...
    if not dist["is_normal"]:
        callout(
            f"Jarque-Bera test rejects normality (p={dist['jb_pvalue']:.4f}). "
            f"Fat tails detected — parametric VaR may underestimate real risk.",
            "warn",
        )
