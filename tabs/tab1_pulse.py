...
from ui.theme import apply_layout, CYAN, YELLOW, RED, GREEN, MUTED


def _fetch_news(symbol: str) -> list[dict]:
    try:
        return yf.Ticker(symbol).news[:3] or []
    except Exception:
        return []
...
def render(data: pd.DataFrame, cfg: dict) -> None:
    ...
    fig.add_trace(go.Scatter(
        x=data["Date"], y=sma(data["close"], 20),
        line=dict(color=YELLOW, width=1.4, dash="dot"), name="20D MA",
    ), row=1, col=1)
        ...
        if news:
            for item in news:
                ...
                st.markdown(
                    ...
                    f'<div class="news-title"><a href="{link}" target="_blank" '
                    f'style="color:#e6edf3;text-decoration:none;">{title}</a></div>'
                    f'<div class="news-meta">{publisher} · {date_str}</div></div>',
                    unsafe_allow_html=True,
                )
        else:
            callout("No recent headlines found.", "info")
    else:
        callout("📰 News available in live mode — disable Demo Mode.", "info")

    query = company_name.replace(" ", "+")
    st.link_button("🔍 More news on Google",
                   f"https://news.google.com/search?q={query}+stock+India&hl=en-IN")
